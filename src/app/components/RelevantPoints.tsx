...
export default function RelevantPoints({ idProceso }: any) {
    ...
    useEffect(() => {
     ...
     const getProcess = async () => {
        const resumeParam = { key: 'id_proceso', paramId: idProceso, urlSlug: "api/consultaProceso" };
        const { data } = await axios.post('/api/auth/endpoint', resumeParam);

        if (data.contenido) {
            setItems(data.contenido);
            localStorage.setItem(`rpoints_${idProceso}`, `${JSON.stringify(data.contenido)}`);//guardar la consulta en el localstorage
